...
const Admin = () => {
  ...
  const analyticsQuery = useQuery({ queryKey: ["admin-analytics"], queryFn: async () => { try { return (await api.get("/admin/analytics")).data; } catch (error) { if (error.response?.status === 404) return { eventTypes: [], popularGames: [], topSearches: [] }; throw error; } } });
  const reload = () => { statsQuery.refetch(); reviewsQuery.refetch(); commentsQuery.refetch(); analyticsQuery.refetch(); };
  ...
  if (statsQuery.isError || reviewsQuery.isError || commentsQuery.isError || analyticsQuery.isError) return <main className="main-container"><div className="state-message error-message"><p>Administrator data could not be loaded.</p><button className="retry-button" onClick={reload}>Try again</button></div></main>;
